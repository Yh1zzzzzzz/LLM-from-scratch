...
def main():
    ...
    parser.add_argument("--using_sft_model", action="store_true", help="Use SFT model as starting point")
    parser.add_argument("--no_using_sft_model", dest="using_sft_model", action="store_false")
    parser.set_defaults(using_sft_model=False)
    ...
    using_sft_model = args.using_sft_model
    if using_sft_model:
        print("Using SFT model as the starting point for GRPO training.")
        name = f"GRPO_with_SFT-{args.epochs}-lr{args.lr}-bs{args.batch_size}"
    else:
        print("Using base LLM model as the starting point for GRPO training.")
        name = f"GRPO_without_SFT-{args.epochs}-lr{args.lr}-bs{args.batch_size}"
    ...
    sft_model_path = BASE_DIR / "sft_model_final" 
    # Hyperparameters
    ...
    for step in range(num_step):
        ...
        model.eval() # 切换到评估模式以禁用 dropout 等
        with torch.no_grad():
        # 将 prompts 扩展 group_size 倍
            repeated_prompts_for_gen = [p for p in batch_prompts for _ in range(group_size)]
        
        # 对 prompts 进行分词
            tokenizer.padding_side = 'left'
            inputs = tokenizer(repeated_prompts_for_gen, return_tensors="pt", padding=True, truncation=True).to(device)
        ...
        # 解码，并只取生成的部分
            ...
            rollout_responses = tokenizer.batch_decode(response_ids, skip_special_tokens=True)
        ...
        for epoch in range(epochs_per_rollout_batch):
            ...
            for i in range(0, rollout_batch_size, train_batch_size):
                ...
                for j in range(0, train_batch_size, micro_train_batch_size):
                    ...
                    notice_str = ""
                    if using_sft_model:
                        notice_str = " (using SFT model)"
                    if not using_sft_model:
                        notice_str = " (not using SFT model)"

                    print(f"  Micro Step {global_step}, Loss: {loss.item():.4f}, Using:{notice_str}")
        ...
        #### 定期评估  ####
        if step >= 0 and step % 5 == 0: # 每 5 个 GRPO step 评估一次
            ...
            with open(generations_output_file, "a", encoding="utf-8") as f:
                for prompt, answer, gold in zip(eval_prompts, generated_answers, gold_answers):
                    reward_info = r1_zero_reward_fn(answer, gold)
                    averaege_eval_reward += reward_info.get("reward", 0.0)
                    record = {"step": step, "prompt": prompt, "generated_answer": answer, "gold_answer": gold, "reward": reward_info.get("reward"), "format_reward": reward_info.get("format_reward"), "answer_reward"  : reward_info.get("answer_reward")}
                    f.write(json.dumps(record, ensure_ascii=False) + "\n")
            averaege_eval_reward /= k
            wandb_data["eval/average_reward"] = averaege_eval_reward 
